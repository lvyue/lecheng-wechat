const [sha1, emitter, xml2js, request, _, utils, crypto] = [require('sha1'), new (require('events')).EventEmitter(), require('xml2js'), require('request'), require('lodash'), require('./utils'), require('crypto')];
// Wechat url config
...
const Wechat = function (options) {
    ...
    this.encrypt = _.isBoolean(options.encrypt) ? options.encrypt : false;
...
Wechat.prototype.handler = function (req, res) {
    ...
    if (req.method.toUpperCase() !== "GET") {
        ...
        if (this.encrypt)
            return res.status(200).end((this.formatDecryptMsg(this.decrypt(_query.echostr, 'base64'))).msg);
